...
use App\Action\Command\Wishlist\WishlistGroup\WishlistGroupMember\RemoveWishlistGroupMemberCommand;
...
final class RemoveWishlistGroupMemberController extends AbstractController
{
    ...
    public function __invoke(Request $request, string $groupId, string $id, CommandBusInterface $commandBus): Response
    {
        if($this->isCsrfTokenValid(sprintf('remove.%s', $id), (string)$request->request->get('_token'))) {
            try {
                $command = new RemoveWishlistGroupMemberCommand();
                $command
